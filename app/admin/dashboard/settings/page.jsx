    ...
    <div className="max-w-[1440px] mx-auto">
      ...
      <div className="mb-8">
        <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
          <div>
            <h1 className="font-inter text-[44.79px] font-black leading-[49.77px] tracking-[-1.48px] text-[#1E293B] mb-2">
              Platform Settings
            </h1>
          ...
          <div className="flex items-center gap-3">
            <motion.button
              ...
              className="px-6 py-2.5 bg-white border border-[#E5E7EB] rounded-lg font-inter text-[14px] font-medium text-[#64748B] hover:bg-[#F8FAFC] transition-colors"
            >
              Cancel
            </motion.button>
            <motion.button
              ...
              className="px-6 py-2.5 bg-[#1E293B] text-white rounded-lg font-inter text-[14px] font-medium hover:bg-[#334155] transition-colors disabled:opacity-50"
            >
        ...
        <div className="flex gap-6">
          {tabs.map((tab) => (
            <button
              ...
              className={`flex items-center gap-2 px-4 py-3 font-inter text-[17.42px] font-bold leading-[26.13px] tracking-[0.26px] border-b-2 transition-colors ${
                activeTab === tab.id
                  ? "border-[#1E293B] text-[#1E293B]"
                  : "border-transparent text-[#64748B] hover:text-[#1E293B]"
              }`}
            >
              <Image src={tab.icon} alt={tab.label} className="h-[18px] w-[18px]" />
              {tab.label}
            </button>
          ))}
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <>
              ...
              <div className="bg-white rounded-xl border border-[#E5E7EB] p-6">
                <div className="flex items-center justify-between mb-6">
                  ...
                  <span className="px-3 py-1 bg-transparent border-[1.24px] border-[#22C55E33] rounded-[12440.4px] font-inter text-[14.93px] font-bold leading-[19.91px] text-[#22C55E] uppercase">
                    SYSTEM ACTIVE
                  </span>
                </div>
                ...
                <div className="space-y-4">
                  {paymentSettings.gateways.map((gateway) => (
                    <div
                      key={gateway.id}
                      className={`flex items-center justify-between p-4 rounded-[9.95px] border-[1.24px] ${
                        gateway.enabled
                    ...
                    >
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <p className="font-inter text-[15px] font-bold text-[#1E293B]">
                            {gateway.name}
                          </p>
                  ...
                  >
                    <div className="flex items-center gap-4">
                      <div className="w-12 h-12 bg-gradient-to-br from-[#9333EA] to-[#4F46E5] rounded-full flex items-center justify-center">
                        <Image src={aiConfigIcon} alt={model.name} className="h-[20px] w-[20px]" />
                      </div>
                      <div>
                        <p className="font-inter text-[15px] font-bold text-[#1E293B]">
                          {model.name}
          ...
          {activeTab === "notifications" && notificationSettings && (
            <div className="bg-white rounded-xl border border-[#E5E7EB] p-6">
              ...
              <div className="space-y-6">
                {/* Email Notifications */}
                <div>
                  <h3 className="font-inter text-[15px] font-bold text-[#1E293B] mb-4">
                    Email Notifications
                  </h3>
                  <div className="space-y-3">
                    {Object.entries(notificationSettings.email).map(([key, value]) => (
                      <div key={key} className="flex items-center justify-between">
                        <span className="font-inter text-[14px] text-[#64748B] capitalize">
        ...
        <div className="space-y-6">
          {/* Quick Actions */}
          <div className="bg-gradient-to-br from-[#273054] to-[#161F42] rounded-xl p-6 text-white">
            <div className="flex items-center gap-2 mb-3">
              <Image src={quickActionsIcon} alt="Quick Actions" className="h-[20px] w-[20px]" />
              <h3 className="font-inter text-[16px] font-bold">Quick Actions</h3>
            </div>
            <p className="font-inter text-[13px] text-[#94A3B8] mb-4">
            ...
            <div className="space-y-2">
              {[
                ...
                <button
                  key={i}
                  className="w-full flex items-center justify-between px-4 py-3 bg-[#334155] hover:bg-[#475569] rounded-[9.95px] border-[1.24px] border-transparent transition-colors group"
                >
                  <span className="font-inter text-[14px]">{action}</span>
                  <Image
                    src={rightArrowIcon}
                    alt=""
                    className="h-[15px] w-[15px] group-hover:translate-x-1 transition-transform"
                  />
                </button>
              ))}
            </div>
          ...
          {/* Security Context */}
          <div className="bg-[#273054] rounded-[14.93px] border-[1.24px] border-[#273054] p-6 text-white opacity-100">
            <div className="flex items-center gap-2 mb-3">
              <Image src={shieldIcon} alt="Security" className="h-[20px] w-[25px]" />
              <h3 className="font-inter text-[19.91px] font-bold leading-[29.86px] text-[#F97316]">
                Security Context
              </h3>
            </div>
            <p className="font-inter text-[13px] text-[#94A3B8] mb-4">
            ...
            <div className="flex items-center gap-2 text-white">
              ...
              <span className="font-inter text-[13px] font-medium">Audit Logging Active</span>
            </div>
          </div>

          {/* Help */}
          <div className="bg-white rounded-[14.93px] border-[1.24px] border-[#314368] p-6">
            <p className="font-inter text-[14px] text-[#64748B] mb-3">
              Need help with fee calculation?
            </p>
            <button className="flex items-center gap-2 font-inter text-[14px] font-medium text-[#3B82F6] hover:underline">
              Read Documentation
