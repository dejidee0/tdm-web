  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto">
        ...
        >
          <div className="flex h-auto lg:h-150">

            {/* ── Panel 1: Left copy ── */}
            <div
              className="hidden lg:flex flex-col justify-center p-8 xl:p-10 w-64 xl:w-72 shrink-0 border-r border-white/06"
            ...
            {/* ── Panel 2: App sidebar ── */}
            <div
              className="hidden sm:flex flex-col w-36 xl:w-40 border-r border-white/06 shrink-0"
            ...
            <div className="flex-1 flex flex-col min-w-0">
              ...
              <div className="flex items-center justify-between px-3 py-2 border-b border-white/06 shrink-0">
                <span className="text-white/25 text-[10px] tracking-widest uppercase font-manrope">
                  3D Preview
                </span>
            ...
            {/* ── Panel 4: Style selector + Estimate ── */}
                  ...
                  >
                    <div
                      className={`w-2 h-2 rounded-full shrink-0 transition-colors ${
                        selectedStyle === style ? "bg-gold" : "bg-white/15"
                      }`}
                    />
        ...
        {/* Mobile copy fallback — shown when panels are hidden */}
        <motion.div
          initial={{ opacity: 0, y: 16 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          transition={{ duration: 0.5 }}
          className="lg:hidden mt-8 space-y-5"
        >
          <span className="text-gold text-[9px] font-bold tracking-[0.2em] uppercase block font-manrope">
            Powered by AI. Designed for You.
          </span>
          <h2 className="font-primary text-2xl font-bold text-white tracking-tight">
          ...
          <Link href="/dashboard/ai-designs">
            <motion.span
              whileHover={{ scale: 1.02 }}
              whileTap={{ scale: 0.97 }}
              className="inline-flex items-center gap-2 px-6 py-3 font-manrope font-bold text-sm tracking-widest uppercase text-black cursor-pointer hover:opacity-90 transition-opacity"
              style={{ background: "linear-gradient(135deg, #D4AF37 0%, #b8962e 100%)" }}
            >
              Try Ziora Now →
            </motion.span>
