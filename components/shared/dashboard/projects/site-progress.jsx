...
export default function SiteProgressPhotos({ photos, isLoading }) {
  ...
  return (
    ...
    >
      ...
      <div className="p-5">
        {isLoading ? (
          ...
          <div className={view === "grid" ? "grid grid-cols-2 gap-4" : "space-y-3"}>
            {items.map((photo, index) => (
              ...
              >
                <div
                  className={view === "grid" ? "absolute inset-0" : "w-32 h-full shrink-0 rounded-xl overflow-hidden"}
                ...
                >
                  <Image
                    ...
                    onError={(e) => {
                      ...
                      e.currentTarget.parentElement.style.backgroundColor = photo.placeholder;
                    }}
                  />
                </div>
        ...
        <button className="w-full mt-4 py-3 text-[13px] font-semibold text-white/35 hover:text-[#D4AF37] border-t border-white/06 transition-colors">
