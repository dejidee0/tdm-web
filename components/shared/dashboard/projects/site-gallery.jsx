...
function PhotoCard({ photo, index, onClick }) {
  ...
  const uploader =
    photo?.uploadedBy ?? photo?.uploader ?? photo?.createdBy ?? "Team";
    ...
    >
      <div className="relative h-44 overflow-hidden bg-white/04">
        ...
        <div className="absolute top-2.5 left-2.5 z-10">
          <span className="text-[16px] font-bold tracking-wider text-white bg-black/60 backdrop-blur-sm px-2 py-0.5 rounded-md">
            {tag}
          </span>
        </div>
      ...
      <div className="p-3.5">
        <div className="flex items-center justify-between mb-1">
          <h3 className="text-[13.5px] font-bold text-white line-clamp-1">
            {title}
          </h3>
          {formattedDate && (
            <span className="text-[11.5px] text-white/30 shrink-0 ml-2">
              {formattedDate}
            </span>
          )}
        </div>
        {description && (
          <p className="text-[11.5px] text-white/40 leading-relaxed mb-3 line-clamp-2">
            {description}
          </p>
        )}
        <div className="flex items-center justify-between mt-2">
          ...
          <button
            className="flex items-center gap-1 text-[11.5px] font-semibold text-[#D4AF37] hover:text-[#D4AF37]/70 transition-colors"
            onClick={(e) => {
              e.stopPropagation();
              onClick(photo);
            }}
...
function SkeletonCard() {
  return (
    <div
      className="rounded-xl border border-white/08 overflow-hidden animate-pulse"
      style={{ background: "#0d0b08" }}
    >
  ...
  return (
    ...
    >
      ...
      >
        ...
        <p className="text-white/70 text-sm font-medium text-center mt-4">
          {title}
        </p>
      </motion.div>
    </motion.div>
...
export default function SiteGallery({ projectId }) {
  ...
  const categories = [
    ...new Set(
      photos.map((p) => p?.tag ?? p?.room ?? p?.category).filter(Boolean),
    ),
  ];
  ...
  const filtered =
    activeFilter === "All Photos"
      ...
      : activeFilter === "Recent"
        ...
        : photos.filter(
            (p) =>
              (p?.tag ?? p?.room ?? p?.category)?.toLowerCase() ===
    ...
    <div className="min-h-screen bg-black p-6 md:p-8">
      <div className="max-w-5xl mx-auto space-y-5">
        ...
        {isLoading ? (
          ...
          <p className="text-white/25 text-[13px] py-12 text-center">
            No photos yet.
          </p>
        ) : (
          <div className="grid grid-cols-3 gap-4">
