...
export async function adminLogin(credentials) {
  try {
    ...
    // A rejected login comes back as HTTP 200 with { success: false, data: null },
    // so the status is not the failure signal — the envelope is. Destructuring
    // data straight away turned a wrong password into a TypeError, which is not
    // an AxiosError, so it fell through to the generic catch below and surfaced
    // as "An unexpected error occurred" instead of the backend's message.
