...
export async function getCurrentVendorUser() {
  try {
    ...
    cookieStore.set("vendorAuthToken", newAccessToken, { ...cookieOpts, httpOnly: true });
    cookieStore.set("vendorBearerToken", newAccessToken, { ...cookieOpts, httpOnly: false });
    if (newRefreshToken) {
      cookieStore.set("vendorRefreshToken", newRefreshToken, { ...cookieOpts, httpOnly: true });
    }
...
export async function vendorLogin(credentials) {
  ...
  try {
    ...
    console.log("🔍 Backend Raw response from vendor login:", JSON.stringify(response, null, 2)); // check shape first
    ...
    cookieStore.set("vendorAuthToken", token, { ...cookieOpts, httpOnly: true, maxAge });

    // Bearer token — readable by client JS so axios can set Authorization header
    cookieStore.set("vendorBearerToken", token, { ...cookieOpts, httpOnly: false, maxAge });
