...
export async function vendorLogin(credentials) {
  try {
    ...
    // apiClient only throws on a non-2xx status, but the backend rejects a login
    // with HTTP 200 and { success: false, data: null }. Destructuring that null
    // threw a TypeError whose raw message was handed straight to the user by the
    // catch below — check the envelope first.
    ...
    // by lib/proxy.js when it authenticates the /api/proxy/v1 mount).
...
export async function vendorLogout() {
  ...
  try {
    const bearerToken = cookieStore.get("vendorAuthToken")?.value;
  ...
  cookieStore.delete("vendorBearerToken"); // legacy readable cookie — see LEGACY_TOKEN_COOKIES
