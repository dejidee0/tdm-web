...
function attachResponseInterceptor(instance) {
  instance.interceptors.response.use(
    ...
    (error) => {
      ...
      if (status) {
        // Extract only a real, user-facing message — never fall back to axios's
        // "Request failed with status code N", which getFriendlyMessage would
        // then show verbatim for an actionable 400.
