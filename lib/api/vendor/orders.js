...
export const vendorOrdersAPI = {
  ...
  createOrder: async (orderData) => {
    const response = await apiClient.post("/api/v1/vendor/orders", orderData);
  ...
  getOrders: async (params = {}) => {
    ...
    const response = await apiClient.get("/api/v1/vendor/orders", { params: queryParams });
    return response.data;
  },

  // GET /api/vendor/orders/:orderId
  getOrderById: async (orderId) => {
    const response = await apiClient.get(`/api/v1/vendor/orders/${orderId}`);
    return response.data;
  },

  // PATCH /api/vendor/orders/:orderId/status
  updateStatus: async (orderId, status) => {
    const response = await apiClient.patch(`/api/v1/vendor/orders/${orderId}/status`, { status });
    return response.data;
  },

  // POST /api/vendor/orders/:orderId/notes
  addNote: async (orderId, note) => {
    const response = await apiClient.post(`/api/v1/vendor/orders/${orderId}/status`, note);
    return response.data;
  },

  // PATCH /api/vendor/orders/:orderId/assignment
  updateAssignment: async (orderId, assignment) => {
    const response = await apiClient.patch(`/api/v1/vendor/orders/${orderId}/assignment`, assignment);
    return response.data;
  },
  ...
  exportOrders: async (params = {}) => {
    ...
    const response = await apiClient.get("/api/v1/vendor/orders/export", {
      params: queryParams,
  ...
  importOrders: async (file) => {
    ...
    const response = await apiClient.post("/api/v1/vendor/orders/import", formData, {
      headers: {
